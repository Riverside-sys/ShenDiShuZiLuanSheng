    ...
    <div class="ui-layer" ref="uiLayer">
      ...
      <div class="bottom-bar">
        <div class="bottom-controls">
          ...
          <button
            class="ctrl-btn"
            :class="{ active: mode === 'overview' || mode === 'overviewTour' }"
            @click="handleOverviewTour"
          >
            总览
          </button>
          <button
            class="ctrl-btn primary"
            @click="handleEnterRoadway"
...
const loadedSegmentCount = computed(
  () => segmentStatus.value.filter((s) => s.loaded).length,
);
...
const updateInitialOverviewCamera = () => {
  ...
  initialOverviewCamera.position.lerp(
    overview.target,
    1 - OVERVIEW_CAMERA_DISTANCE_SCALE,
  );
  ...
  controls.maxDistance = Math.max(
    initialOverviewCamera.position.distanceTo(overview.target) * 8,
    5000,
  );
};
...
const setupRenderer = () => {
  ...
  scene.fog = new THREE.FogExp2(0x06121f, 0.00035);
  ...
  // 贴色巷道以顶点色为主，用偏亮的环境光保证暗岩壁仍可读
  scene.add(new THREE.AmbientLight(0xffffff, 1.35));
  const hemiLight = new THREE.HemisphereLight(0xf2f7ff, 0x6a7378, 1.05);
  scene.add(hemiLight);
  const dl1 = new THREE.DirectionalLight(0xffffff, 1.7);
  dl1.position.set(500, 800, 300);
  scene.add(dl1);
  const dl2 = new THREE.DirectionalLight(0xe8f0ff, 1.05);
  dl2.position.set(-300, -200, -500);
  scene.add(dl2);
  const dl3 = new THREE.DirectionalLight(0xfff6e8, 0.85);
  dl3.position.set(0, 200, -800);
  scene.add(dl3);
...
const loadMergedMesh = async () => {
  ...
  try {
    ...
    mergedBBox =
      ...
      new THREE.Box3().setFromObject(mergedPoints);
...
const computeCenterline = (geometry: THREE.BufferGeometry) => {
  ...
  if (mergedSize.x >= mergedSize.y && mergedSize.x >= mergedSize.z)
    longAxis = "x";
...
const createOverviewProbeCamera = () => {
  if (!camera) return null;
  const probe = new THREE.PerspectiveCamera(
    initialOverviewCamera.fov,
    camera.aspect,
    Math.max(initialOverviewCamera.near, 0.1),
    Math.max(initialOverviewCamera.far, 1000),
  );
  probe.position.copy(initialOverviewCamera.position);
  probe.up.copy(initialOverviewCamera.up);
  probe.lookAt(initialOverviewCamera.target);
  probe.updateMatrixWorld(true);
  probe.updateProjectionMatrix();
  return probe;
};

/** 保证中线 t=0 在屏幕左侧、t=1 在右侧 */
const orientCenterlineToScreenLeft = () => {
  if (!centerlineCurve) return;
  const probe = createOverviewProbeCamera();
  if (!probe) return;

  const ndc0 = centerlineCurve.getPoint(0).clone().project(probe);
  const ndc1 = centerlineCurve.getPoint(1).clone().project(probe);
  if (ndc0.x <= ndc1.x) return;

  const samples = centerlineCurve.getPoints(120);
  samples.reverse();
  centerlineCurve = new THREE.CatmullRomCurve3(
    samples,
    false,
    "catmullrom",
    0.5,
...
const flyToSegmentExternal = (id: number) => {
  if (!camera || !controls || !centerlineCurve) return;

  if (mode.value !== "overview" && mode.value !== "overviewTour") {
    setMode("overview");
  } else if (controls) {
    controls.enabled = true;
  }

  const tMid = (id - 0.5) / SEGMENT_COUNT;
  const target = centerlineCurve.getPoint(tMid).add(roadwayOffset);

  const viewDir = initialOverviewCamera.position
    .clone()
    .sub(initialOverviewCamera.target);
  if (viewDir.lengthSq() < 1e-6) {
    viewDir.set(0.35, -0.75, 0.55);
  }
  viewDir.normalize();

  const bandLen = Math.max(centerlineCurve.getLength() / SEGMENT_COUNT, 1);
  const dist = Math.max(bandLen * 5.2, mergedSize.length() * 0.14, 40);
  const endPos = target.clone().addScaledVector(viewDir, dist);

  flyTo(endPos, target, 1100);
};
...
const buildAutoPath = (outside = false) => {
  ...
  if (outside && centerlineCurve) {
    ...
    for (let i = 0; i <= steps; i++) {
      ...
      autoPath.push({
        pos: initialOverviewCamera.position.clone(),
        target: center,
      });
    }
    return;
...
const startAuto = (outside = false, nextMode: SceneMode = "auto") => {
  ...
  autoDuration =
    nextMode === "overviewTour" ? OVERVIEW_TOUR_DURATION : INSPECTION_DURATION;
  mode.value = nextMode;
...
const tickAuto = () => {
  if (
    (mode.value !== "auto" && mode.value !== "overviewTour") ||
    !camera ||
    autoPath.length < 2
  )
    return;
  ...
  const seg = isInspection
    ? segmentStatus.value[Math.min(Math.floor(t * 11), 10)]
    : null;
...
const tickRoam = (dt: number) => {
  ...
  const speed =
    (keyState["shift"] ? roamUnitsPerSecFast : roamUnitsPerSec) * dt;
