...
class Popup {
  ...
  add(conf) {
      ...
      ctn.id = id;
      // console.log("viewer----js",_this.viewer)
  ...
  render() {
      ...
      for (var c in _this.ctnList) {
          ...
          var position = Cesium.SceneTransforms.worldToWindowCoordinates(_this.viewer.scene, _this.ctnList[c][0]);
          ...
          if (position && position.x && position.y) {
              if (Math.abs(position.x) > (window.innerWidth * 2) || Math.abs(position.y) > (window.innerHeight * 2)) {
                  ...
                  _this.ctnList[c][1].style.top = (position.y-70) + "px";
              }
          }
      }
  }
